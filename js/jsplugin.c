  ...
  if (tx_prec == GKS_K_TEXT_PRECISION_STRING)
    {
      ...
      if (gkss->txal[0] == GKS_K_TEXT_HALIGN_CENTER)
        {
          js_text(xstart, ystart, nchars, chars, size, gkss->chh, p->angle, bold, italic, 1, gkss->txal[1], p->family,
                  p->rgb[p->color]);
        }
      else if (gkss->txal[0] == GKS_K_TEXT_HALIGN_RIGHT)
        {
          js_text(xstart, ystart, nchars, chars, size, gkss->chh, p->angle, bold, italic, 2, gkss->txal[1], p->family,
                  p->rgb[p->color]);
        }
      else
        {
          js_text(xstart, ystart, nchars, chars, size, gkss->chh, p->angle, bold, italic, 0, gkss->txal[1], p->family,
                  p->rgb[p->color]);
        }
    }
  else
  ...
  if (!true_color)
    {
      for (i = 0; i < width * height; i++)
        {
          ind = colia[i];
          if (ind >= MAX_COLOR) ind = MAX_COLOR - 1;
          colia[i] = (int)(p->rgb[ind][0] << 0) + ((int)(p->rgb[ind][1]) << 8) + ((int)(p->rgb[ind][2]) << 16) +
                     ((int)(p->rgb[ind][3]) << 24);
        }
    }
