#ifndef GLGR_H_INCLUDED
#define GLGR_H_INCLUDED
#include <QGLWidget>
#include <vector>
#include "vec.h"
#include "rect.h"
...
private:
  void setBoundaries(float xmin_, float xmax_, float ymin_, float ymax_, float zmin_, float zmax_, float cmin_=-1, float cmax_=-1);
...
public slots:
  ...
  void setXTick(int value)
  {
    setXTick((float) value);
  }
  void setYTick(int value)
  {
    setYTick((float) value);
  }
  void setZTick(int value)
  {
    setZTick((float) value);
  }
...
private:
  ...
  //flags
  bool refreshSurfaceVertices,refreshSurfaceIndices, refreshSurfaceColors, refreshGridIndices,
       refreshContourVertices, refreshContourIndices,refreshContourLineVertices;
