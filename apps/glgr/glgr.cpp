...
void GLGrWidget::drawTexturedRect(const vec& pos, vec xdir, vec ydir, 
  ...
  glBegin(GL_QUADS);
  glTexCoord2f(0.0,0.0);glVertex3fv((pos-xdir*width/2-ydir*height/2).getv());
  glTexCoord2f(0.0,ycut);glVertex3fv((pos-xdir*width/2+ydir*height/2).getv());
  glTexCoord2f(xcut,ycut);glVertex3fv((pos+xdir*width/2+ydir*height/2).getv());
  glTexCoord2f(xcut,0.0);glVertex3fv((pos+xdir*width/2-ydir*height/2).getv());
  glEnd();
...
void GLGrWidget::provideCorrectDirections(const vec& r1, const vec& r2, vec& nr1, vec& nr2,GLfloat camMat[4][4]) {
  vec t1,t2;
  nr1=r1;nr2=r2;
  t1=nr1.transform2d(camMat);t2=nr2.transform2d(camMat);
  if(t2.y()<0) {
    nr2*=-1;
...
void GLGrWidget::get_plot3d_orders(QString &data_string) {
  ...
  if (gr_root.tagName() != "gr") {
      QMessageBox::information(NULL, QObject::tr("DOM gr"),
                                 QObject::tr("The file is not an XML gr file."));
        return;
    } else if (gr_root.hasAttribute("version")
               && gr_root.attribute("version") != "1.0") {
      QMessageBox::information(NULL, QObject::tr("DOM gr"),
  ...
  if ( hasChilds ) {
    ...
    n_gr_rootChilds = gr_rootChilds.count();
      ...
      QMessageBox::information(NULL, QObject::tr("DOM gr"),
			       QObject::tr("The XML file has no child nodes."));
       return;
  }
  ...
  for( int i=0; i<n_gr_rootChilds; i++) {
    ...
    if( node_name == QString("axes3d") ) {
	  ...
	  if( int_value & LOGZ )  { setLogZ( true );  } else { setLogZ( false );  }
       ...
       if( namedNodeMap.contains("option") )
        {
         ...
         QMessageBox::information(NULL, QObject::tr("DOM gr"),
                                 QObject::tr("no attribute <option>."));
        return;
       }
