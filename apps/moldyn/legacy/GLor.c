 ...
 */
#define GLORInitStruct_INITIALIZER {512,512}
 ...
 * ::_GLORContextStruct_.
 */
#define GLORContextStruct_INITIALIZER {GLORInitStruct_INITIALIZER,0,0,\
...
static void      glorDoDrawMesh_(GLORMesh mesh, 
                                unsigned int n, const double *positions, 
                                const double *directions, const double *ups, 
                                const double *colors, const double *scales);
...
GLORAPI GLORError glorInit(GLORInitAttribute *attrib_list) {
    ...
    if (!strncmp((const char *)glGetString(GL_VERSION),"3.",2) || 
            glorExtensionSupported_("GL_ARB_framebuffer_object")
       ) {
        ...
        if (context_struct_.fbo_is_initialized) {
            ...
            for (i = 0; i < context_struct_.mesh_list_capacity_; i++) {
                if (context_struct_.mesh_list_[i].data.display_list_id != 0) {
                    glDeleteLists(
                        context_struct_.mesh_list_[i].data.display_list_id,1);
 ...
 */
GLORAPI GLORError glorCreateMesh(GLORMesh *mesh, unsigned int n, const double *vertices, 
                        const double *normals, const double *colors) {
    ...
    *mesh = context_struct_.mesh_list_first_free_;
    if (context_struct_.mesh_list_first_free_ >= context_struct_.mesh_list_capacity_) {
        ...
        while(context_struct_.mesh_list_capacity_ < new_capacity) {
            ...
            context_struct_.mesh_list_[context_struct_.mesh_list_capacity_].data.vertex_buffer_object.number_of_vertices = 0;
 ...
 */
GLORAPI void glorDrawMesh(GLORMesh mesh, unsigned int n, const double *positions, 
                  const double *directions, const double *ups, 
                  const double *colors, const double *scales) {
 ...
 */
GLORAPI void glorCameraLookAt(double camera_x, double camera_y, double camera_z, 
 ...
 */
GLORAPI GLORError glorCameraProjectionParameters(double vertical_field_of_view, 
...
static void glorDraw_(GLuint width, GLuint height) {
    ...
    */
    ...
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    {
        ...
        draw = context_struct_.draw_list_;
        while (draw) {
            glorDoDrawMesh_(draw->mesh,draw->n,draw->positions,draw->directions,
 ...
 */
GLORAPI GLORError glorGetPixmap(GLORPixel *bitmap, unsigned int width, 
                        unsigned int height) {
    unsigned int x, y;
...
static char *error_strings_[] = {
    "kGLORENoError",
    "kGLOREInvalidValue",
    "kGLOREInvalidAttribute",
    "kGLOREInitError",
    "kGLOREOpenGLError",
    "kGLOREOutOfMemory",
    "kGLORENotInitialized",
    "kGLORECameraNotInitialized",
    "kGLOREUnknownError"
};
 ...
 * \sa glorDrawMesh()
 */
GLORAPI void glorDrawCylinderMesh(unsigned int n, const double *positions, const double *directions, const double *colors, const double *radii, const double *lengths) {
    unsigned int i;
    unsigned int j;
 ...
 * \sa glorDrawMesh()
 */
GLORAPI void glorDrawConeMesh(unsigned int n,const double *positions, const double *directions, const double *colors, const double *radii, const double *lengths) {
    unsigned int i;
    unsigned int j;
    unsigned int min_index;
    unsigned int min_n;
