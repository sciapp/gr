...
GLGrWidget::GLGrWidget(QWidget *parent) :
  ...
  paint_surface(false), paint_contour(false), paint_contour_lines(false),paint_axes(false),paint_arrows(false),
  paint_legend(false), autoscale(false),flipx(false), flipy(false), flipz(false),
  xytype(0),reduction_rate(100), gridColor(0),logX(false), logY(false),logZ(false),useCCdata(false),
  x_tick(0), y_tick(0), z_tick(0), tick_size(.02f),xmin(0),xmax(100),ymin(0),ymax(100),zmin(0),zmax(100),
  x_major_tick(0),  y_major_tick(0),  z_major_tick(0),
  paintXAxis(false), paintYAxis(false), paintZAxis(false), paintCustomizedXLabel(false),
  paintCustomizedYLabel(false), paintCustomizedZLabel(false),
  refreshXTextures(false), refreshYTextures(false), refreshZTextures(false),rotation(0), tilt(0),
  data3D(NULL), dataCC(NULL),source_data3D(NULL),source_dataCC(NULL),xdim(0),ydim(0),source_xdim(0),source_ydim(0),
  ...
  surfaceVertices(NULL),surfaceIndices(NULL),surfaceColors(NULL),gridIndices(NULL),
  contourVertices(NULL),contourIndices(NULL),ct(NULL), contourLineNumber(5)
{
  QPalette palette;
  palette.setColor(backgroundRole(), QColor(Qt::white));
  setPalette(palette);
  setGeometry(10,10,600,600);
  setWindowTitle(tr("GLGr"));
  createColorTable(0);
...
void GLGrWidget::setBoundaries(float xmin_, float xmax_, float ymin_, float ymax_, float zmin_, float zmax_, float cmin_, float cmax_)
{
  if (xmax_==xmin_)
    xmax_+=1;
  if (ymax_==ymin_)
...
void GLGrWidget::setTitle(const QString  t)
{
  if (t!=title)
    {
      title=t;
...
void GLGrWidget::setSubTitle(const QString  t)
{
  if (t!=subtitle)
    {
      subtitle=t;
...
void GLGrWidget::paintGL()
{
  ...
  upper_offset=0;
  if (title!="")
    upper_offset+=title_offset;
  if (subtitle!="")
    upper_offset+=subtitle_offset;
  left_offset=0;
  if (paint_legend || paint_arrows)
    left_offset=legend_offset;
  glOrtho(.5-proj_rad-left_offset, .5+proj_rad, .5-proj_rad-.08, .5+proj_rad+.08+upper_offset, -100,100);
  ...
  if ((refreshXTextures && paintXAxis && paint_axes) || last_context!=context())
    {
      createTextureArray(xmin,xmax,x_tick,x_major_tick,x_textures, paintCustomizedXLabel?&x_indiv_label:NULL,logX);
      refreshXTextures=false;
    }
  if ((refreshYTextures && paintYAxis && paint_axes) || last_context!=context())
    {
      createTextureArray(ymin,ymax,y_tick,y_major_tick,y_textures,paintCustomizedYLabel?&y_indiv_label:NULL,logY);
      refreshYTextures=false;
    }
  if ((refreshZTextures && paintZAxis && paint_axes) || last_context!=context())
    {
      createTextureArray(zmin,zmax,z_tick,z_major_tick,z_textures,paintCustomizedZLabel?&z_indiv_label:NULL,logZ);
  ...
  if (paint_surface || xytype>0)
    paintSurface();
  if ((xytype>0 || paint_surface) && (xytype==1 || xytype==2))
    paintGrid();
  if (paint_contour)
    paintContour();
        ...
        for (int i=0; i<xdim; ++i)
          {
            ...
            if (flipz)
              surfaceVertices[index]=surfaceVertices[index] * -1 +1;
            index++;
...
void GLGrWidget::createSurfaceColors()
{
  ...
  if (maximum_too_low)
    cerr << "warning, maximum for color_data too low\n";
}
  ...
  for (k=1; k<contourLineNumber+1; ++k)
    {
      ...
      for (j=ydim-1; j>=0; --j)
        for (i=0; i<xdim; ++i)
          ct[i][j] = colorData(i,j) >= value ? ct_upper : ct_lower;
      for (j=0; j<ydim; ++j)
        for (i=0 ; i<xdim; ++i)
          {
        ...
        if (y+1<ydim)
          {
            if (ct[x][y+1] == ct_lower)
              {
                contourLineVertices.push_back(vec(logXValue((float)x), 0, logYValue((float)y + (colorData(x,y)-value) / (colorData(x,y) - colorData(x,y+1)))));
        ...
        if (x-1>=0)
          {
            if (ct[x-1][y] == ct_lower)
              {
                contourLineVertices.push_back(vec(logXValue((float)x -(colorData(x,y)-value) / (colorData(x,y) - colorData(x-1,y))), 0, logYValue((float)y)));
...
void GLGrWidget::createTextureArray(float min,float max,float tick,int major, vector<TickTexture> & textures, vector<CustomizedLabel> *indiv,bool is_log)
{
...
{
  if (reduce3D && data3D && data3D != source_data3D)
    delete [] data3D;
  if (dataCC && dataCC != source_dataCC)
    delete [] dataCC;
...
void GLGrWidget::paintSurface()
{
  ...
  for (int i=0; i<ydim-1; ++i)
    glDrawElements(GL_TRIANGLE_STRIP, xdim*2,GL_UNSIGNED_INT,surfaceIndices[i]);
...
void GLGrWidget::paintGrid()
{
  ...
  if (xytype==0 || xytype==3)
    for (j=0; j<ydim-1; ++j)
      glDrawElements(GL_QUAD_STRIP, xdim*2,GL_UNSIGNED_INT,gridIndices[j]);
  else
    {
      for (j=0; j<ydim; ++j)
        glDrawElements(GL_LINE_STRIP, xdim,GL_UNSIGNED_INT,gridIndices[j]);
      for (j=ydim; j<ydim+2; ++j)
        glDrawElements(GL_LINE_STRIP, ydim,GL_UNSIGNED_INT,gridIndices[j]);
    }
  glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
  glDisableClientState(GL_VERTEX_ARRAY);
}
...
void GLGrWidget::paintAxes()
{
  ...
  if (test_pos.y()==.5 || test_pos.z()==-.5)
    tick_direction*=-1;
  if (paintXAxis)
    {
      paintAxis(start,vec(1,0,0),tick_direction,x_tick, x_major_tick, tick_size,xmin,xmax,x_textures,flipx, logX);
      if (x_axis_label!="")
        {
          ...
          label_texture.width = label_texture.height / label_texture.texture.textHeight * label_texture.texture.textWidth;
          ...
          drawTexturedRect(pos,d1, d2, label_texture.width, label_texture.height,label_texture.xcut,label_texture.ycut);
  ...
  test_pos=tick_direction/2+start;
  if (test_pos.x()==.5 || test_pos.y()==.5)
    tick_direction*=-1;
  if (paintYAxis)
    {
      paintAxis(start,vec(0,0,-1),tick_direction,y_tick, y_major_tick, tick_size,ymin,ymax,y_textures, flipy, logY);
      if (y_axis_label!="")
        {
  ...
  if (test_pos.x()==.5 || test_pos.z()==-.5)
    tick_direction*=-1;
  if (paintZAxis)
    {
      paintAxis(start_z,vec(0,1,0),tick_direction,z_tick,z_major_tick,tick_size,zmin,zmax,z_textures, flipz, logZ);
      if (z_axis_label!="")
        {
                           ...
                           float tick_size, float min, float max,  const vector<TickTexture> & textures, bool flip, bool is_log)
{
  ...
  glEnd();
  // draw ticks
  if (!is_log)    // linear mode
    {
      first_tick_value = (((int) (min/tick)))*tick;
      if (first_tick_value<min)
        first_tick_value+=tick;
      len = max - min;
    ...
    for (int i=0; i<(int)rect_vector.size(); i+=step)
      {
        ...
        drawTexturedRect(rect_vector[i].pos(),r1,r2,textures[i].width,textures[i].height,textures[i].xcut,textures[i].ycut);
      }
}
  ...
  if (title!="")
    {
      ...
      drawTexturedRect(pos,vec(1,0,0), vec(0,1,0), title_texture.width, title_texture.height,title_texture.xcut,title_texture.ycut);
...
void GLGrWidget::drawTexturedRect(const vec& pos, vec xdir, vec ydir,
                                  float width, float height, float xcut, float ycut)
{
...
void GLGrWidget::makeImage(QString str,  GLubyte **texData, Texture *tex,const QColor& color)
{
  ...
  tex->textWidth = fm.width(str);
  tex->textHeight  = fm.height();
  tex->width = make_pot(tex->textWidth);
  tex->height = make_pot(tex->textHeight);
  ...
  for (int i=0; i<texImage->height()*texImage->width(); ++i)
    {
      ...
      if ((int)(*texData)[i*4  ] == 255 &&
          (int)(*texData)[i*4+1] == 255 &&
          (int)(*texData)[i*4+2] == 255)
        {
#ifdef WIN32
  ...
  if (printdialog.exec() == QDialog::Accepted)
    {
      ...
      for (it=x_textures.begin(); it!=x_textures.end(); ++it)
        glDeleteTextures(1,&(it->texture.texName));
      for (it=y_textures.begin(); it!=y_textures.end(); ++it)
        glDeleteTextures(1,&(it->texture.texName));
      for (it=z_textures.begin(); it!=z_textures.end(); ++it)
        glDeleteTextures(1,&(it->texture.texName));
      x_textures.clear();
      y_textures.clear();
      z_textures.clear();
      ...
      if (printer->height() < pix_number)
        pix_number=printer->height();
      ...
      // i have to clear the texture_lists in order to prevent eventually erasing wrong textures of the GLWidget Context,
      // when new textures are constructed and the old ones in the texture_lists are erased.
      x_textures.clear();
      y_textures.clear();
      z_textures.clear();
    }
...
size_t GLGrWidget::getNextTag(const QString &txt,QString &attribute, int pos)
{
  int new_pos;
  if (txt[pos]==' ')
    pos++;
  if (txt[pos]==QChar('"'))
    {
      pos++;
      new_pos=txt.indexOf(QRegExp("[^\\\\]\""),pos);
      if (new_pos==-1)
        {
          cerr << "missing closing \"\n";
          new_pos=txt.length();
        }
      attribute=txt.mid(pos,new_pos-pos+1);
      attribute.remove('\\');
    }
  else
    {
      ...
      if (new_pos==-1)
        new_pos=txt.length();
      attribute=txt.mid(pos,new_pos-pos);
    }
  return new_pos+1;
}
...
void GLGrWidget::get_plot3d_orders(QString &data_string)
{
  ...
  float float_value;
  int int_value;
//  std::cout << "entry glgr.cpp get_plot3d_orders: data_string = " << qPrintable(data_string) << std::endl;
  data_string=data_string.simplified();
  while ((pos=data_string.indexOf(':',last_pos))!=-1)
    {
      error=false;
      attribute=data_string.mid(last_pos,pos-last_pos);
      ...
      if (attribute=="data3d")
        {
          int x,y;
          float *d;
          last_pos=getNextTag(data_string,attribute,last_pos);
          sscanf(attribute.toLatin1(),"%d",&x);
          last_pos=getNextTag(data_string,attribute,last_pos);
          sscanf(attribute.toLatin1(),"%d",&y);
          d=new float[x*y];
          for (int i=0; i<x*y; ++i)
            {
              last_pos=getNextTag(data_string,attribute,last_pos);
              if (sscanf(attribute.toLatin1(),"%f", d+i)!=1)
                {
                  cerr << "error in data3d: not enough values" << endl;
                  error=true;
                  break;
                }
            }
          ...
          for (int i=0; i<x*y; ++i)
            {
              ...
              if (sscanf(attribute.toLatin1(),"%f", d+i)!=1)
                {
                  cerr << "error in data4d: not enough values" << endl;
                  error=true;
                  break;
                }
            }
      ...
      else if (attribute==QString("flipx"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setFlipX(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("flipy"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setFlipY(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("flipz"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setFlipZ(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("title"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setTitle(attribute);
        }
      else if (attribute==QString("subtitle"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setSubTitle(attribute);
        }
      else if (attribute==QString("xmin"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          float_value = attribute.toFloat();
          setXMin(float_value);
        }
      ...
      else if (attribute==QString("zmin"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          float_value = attribute.toFloat();
          setZMin(float_value);
      ...
      else if (attribute==QString("cmin"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          float_value = attribute.toFloat();
          setCMin(float_value);
        }
      else if (attribute==QString("cmax"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          float_value = attribute.toFloat();
          setCMax(float_value);
        }
      else if (attribute==QString("xtick"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          float_value = attribute.toFloat();
      ...
      else if (attribute==QString("xlabel"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setXLabel(attribute);
        }
      else if (attribute==QString("ylabel"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setYLabel(attribute);
        }
      else if (attribute==QString("zlabel"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setZLabel(attribute);
        }
      ...
      else if (attribute==QString("numclines"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          int_value = attribute.toInt();
          setContourLineNumber(int_value);
        }
      else if (attribute==QString("surface"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setPaintSurface(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("contour"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setPaintContour(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("colormap"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          int_value = attribute.toInt();
          if (int_value>12 || int_value < 0)
            int_value=0;
          setColormap(int_value);
        }
      else if (attribute==QString("clines"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setPaintContourLines(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("axes"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setPaintAxes(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("orientation"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setPaintOrientation(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("legend"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setPaintLegend(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("logx"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setLogX(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("logy"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setLogY(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("logz"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setLogZ(attribute[0].toLatin1()-'0');
        }
      else if (attribute==QString("xytype"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          int_value = attribute.toInt();
          setXYType(int_value);
        }
      else if (attribute==QString("reduction_rate"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          int_value = attribute.toInt();
          setReductionRate(int_value);
        }
      else if (attribute==QString("gridcolor"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          int_value = attribute.toInt();
          setGridColor(int_value);
        }
      else if (attribute==QString("print"))
        {
          print();
        }
      else if (attribute==QString("customized"))
        {
          int axis;
          float value;
          last_pos=getNextTag(data_string,attribute,last_pos);
          axis=attribute[0].toLatin1()-'x';
          last_pos=getNextTag(data_string,attribute,last_pos);
          value = attribute.toFloat();
          last_pos=getNextTag(data_string,attribute,last_pos);
          addCustomizedLabel(axis,value,attribute.toLatin1());
          if (data_string[last_pos]=='\n')
            last_pos++;
          setPaintCustomizedLabel(axis,true);
          if (axis==0)
            refreshXTextures=true;
          else if (axis==1)
            refreshYTextures=true;
      ...
      else if (attribute==QString("useCCdata"))
        {
          last_pos=getNextTag(data_string,attribute,last_pos);
          setUseCCdata(attribute[0].toLatin1()-'0');
        }
      else
        {
          if (attribute.length() >= 19)
            {
              attribute[16] = '.';
              attribute[17] = '.';
              attribute[18] = '.';
          ...
          cerr << "unknown resource: " << qPrintable(attribute) << endl;
        }
      last_pos=data_string.indexOf(';',last_pos-1)+1;
      if (last_pos==0)
        break;
    }
  update();
