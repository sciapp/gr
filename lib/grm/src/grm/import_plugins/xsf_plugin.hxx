...
public:
  ...
  GRM_EXPORT grm_error_t readDataFile(const std::string &path,
                                      std::vector<std::vector<std::vector<std::vector<double>>>> &data,
                                      std::vector<std::vector<int>> &x_data, std::vector<std::vector<int>> &y_data,
                                      std::vector<std::vector<int>> &error_data,
                                      std::vector<std::vector<std::string>> &labels, std::vector<grm_args_t *> &args,
                                      const char *colms, const char *x_colms, const char *y_colms, const char *e_colms,
                                      std::vector<PlotRange> &ranges,
                                      std::vector<grm_special_axis_series_t *> &special_axis_series,
                                      InputFlags &input_flags, std::vector<std::vector<int>> &timestamps,
                                      std::vector<double *> special_data_grids) override;
