...
static void imshow(const std::shared_ptr<GRM::Element> &element, const std::shared_ptr<GRM::Context> &context)
{
  ...
  if (!element->hasAttribute("shape")) throw NotFoundError("Imshow series is missing required attribute shape-data.\n");
...
static void triSurface(const std::shared_ptr<GRM::Element> &element, const std::shared_ptr<GRM::Context> &context)
{
  ...
  int nz = pz_vec.size();
  if (nx != ny || nx != nz)
    throw std::length_error("For trisurface series x-, y- and z-data must have the same size.\n");
