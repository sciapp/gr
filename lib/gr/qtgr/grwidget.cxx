...
void InteractiveGRWidget::paintEvent(QPaintEvent *event) {
    ...
    if (mwidth > mheight) {
        vp_width  = mheight;
        vp_height = mheight;
        gr_setwsviewport((mwidth - mheight) / 2,
                         mwidth - (mwidth - mheight) / 2, 0, mheight);
        ...
        vp_width  = mwidth;
        vp_height = mwidth;
        gr_setwsviewport(0, mwidth, (mheight - mwidth) / 2,
                         mheight - (mheight - mwidth) / 2);
        transx = 0;
...
void InteractiveGRWidget::mouseMoveEvent(QMouseEvent *event) {
    ...
    qobject_cast<QMainWindow *>(parent())->statusBar()->showMessage(QString(
                                tr("(%1, %2)")).arg(x).arg(y));
    if (event->buttons() == Qt::LeftButton) {
        rubberBand->setGeometry(QRect(this->drag_start,
...
void InteractiveGRWidget::mouseReleaseEvent(QMouseEvent *event) {
    ...
    if (leftButton && duration > 0.1) {
        zoom_rect = QRect(0, 0, this->width(),
                          this->height()).intersected(rubberBand->geometry());
...
void InteractiveGRWidget::wheelEvent(QWheelEvent *event) {
    ...
    gr_ndctowc(&x, &y);
    gr_setwindow(x - (x - xmin) * fac, x + (xmax - x) * fac,
                 y - (y - ymin) * fac, y + (ymax - y) * fac);
    this->repaint();
}
