...
public:
  ...
  {
    {
      auto it = paintersAndImages_.find(id);
      if (it != paintersAndImages_.end())
        {
    ...
    image->setDevicePixelRatio(parentPainter_.device()->devicePixelRatioF());
#elif QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
    image->setDevicePixelRatio(parentPainter_.device()->devicePixelRatio());
#endif
        ...
        for (unsigned int x = 0; x < paintDevice.width(); x++)
          {
            ...
            if ((pixels[y * paintDevice.width() + x]) != 0xFFFFFFFF)
              {
