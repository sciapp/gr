...
#define DLLEXPORT __declspec(dllexport)
  ...
  DLLEXPORT void gks_htmplugin(int fctid, int dx, int dy, int dimx, int *i_arr, int len_f_arr_1, double *f_arr_1,
...
static void htm_memcpy(HTM_stream *p, char *s, size_t n)
{
...
static void htm_write(const char *args, ...)
{
  va_list ap;
...
static void htm_write_footer(const char *args, ...)
{
  va_list ap;
  char fmt[BUFSIZ], s[BUFSIZ];
...
static HTM_stream *htm_alloc_stream(void)
{
  HTM_stream *p;
...
static void polymarker(int n, double *px, double *py)
{
  ...
  htm_write("c.fillStyle = \"rgba(%d,%d,%d,%f)\";\n", p->rgb[mk_color][0], p->rgb[mk_color][1], p->rgb[mk_color][2],
            p->transparency);
...
void mem_png_write_data(png_structp png_ptr, png_bytep data, png_size_t length)
{
  ...
  if (!mem_png->buffer) png_error(png_ptr, "Write Error");
...
static void image_routine(double xmin, double xmax, double ymin, double ymax, int dx, int dy, int dimx, int *colia,
                          int true_color)
{
...
static void draw_image(double xmin, double xmax, double ymin, double ymax, int dx, int dy, int dimx, int *colia)
{
...
static void set_clip_rect(int tnr)
{
  ...
  htm_write("c.rect(%.3f, %.3f, %.3f, %.3f);\n", p->rect[tnr][0][0], p->rect[tnr][0][1],
            p->rect[tnr][1][0] - p->rect[tnr][0][0], p->rect[tnr][1][1] - p->rect[tnr][0][1]);
  htm_write("c.clip();\n");
}

static void set_clipping(int clip)
{
  gkss->clip = clip;
...
static void fillarea(int n, double *px, double *py)
{
  ...
  htm_write("c.fillStyle = \"rgba(%d,%d,%d,%f)\";\n", p->rgb[fl_color][0], p->rgb[fl_color][1], p->rgb[fl_color][2],
            p->transparency);
...
static void text(double px, double py, int nchars, char *chars)
{
  ...
  htm_write("c.fillStyle = \"rgba(%d,%d,%d,%f)\";\n", p->rgb[tx_color][0], p->rgb[tx_color][1], p->rgb[tx_color][2],
            p->transparency);
  htm_write("c.strokeStyle = \"rgba(%d,%d,%d,%f)\";\n", p->rgb[tx_color][0], p->rgb[tx_color][1], p->rgb[tx_color][2],
            p->transparency);
...
static void polyline(int n, double *px, double *py)
{
  ...
  htm_write("c.strokeStyle = \"rgba(%d,%d,%d,%f)\";\n", p->rgb[ln_color][0], p->rgb[ln_color][1], p->rgb[ln_color][2],
            p->transparency);
...
static void set_window(int tnr, double xmin, double xmax, double ymin, double ymax)
{
...
void gks_htmplugin(int fctid, int dx, int dy, int dimx, int *ia, int lr1, double *r1, int lr2, double *r2, int lc,
                   char *chars, void **ptr)
{
  int i;
  ...
  switch (fctid)
    ...
    case 3:
      ...
      if (p->conid != 0) close_socket(p->conid);
    ...
    case 16:
      /* cell array */
      if (p->state == GKS_K_WS_ACTIVE) cellarray(r1[0], r1[1], r2[0], r2[1], dx, dy, dimx, ia);
      break;

    case 48:
      /* set color representation */
      if (p->state == GKS_K_WS_ACTIVE) set_color_rep(ia[1], r1[0], r1[1], r1[2]);
      break;

    case 49:
      /* set window */
      if (p->state == GKS_K_WS_ACTIVE) set_window(gkss->cntnr, r1[0], r1[1], r2[0], r2[1]);
      break;

    case 50:
      /* set viewport */
      if (p->state == GKS_K_WS_ACTIVE) set_viewport(gkss->cntnr, r1[0], r1[1], r2[0], r2[1]);
      break;

    case 52:
      /* select normalization transformation */
      if (p->state == GKS_K_WS_ACTIVE) select_xform(ia[0]);
      break;

    case 53:
      /* set clipping inidicator */
      if (p->state == GKS_K_WS_ACTIVE) set_clipping(ia[0]);
      break;

    case 201:
      /* draw_image */
      if (p->state == GKS_K_WS_ACTIVE) draw_image(r1[0], r1[1], r2[0], r2[1], dx, dy, dimx, ia);
      break;

    case 203:
      /* set transparency */
      if (p->state == GKS_K_WS_ACTIVE) set_transparency(r1[0]);
      break;
    }
}
