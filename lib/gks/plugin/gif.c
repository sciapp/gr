  ...
  if (num_colors == 1)
  ...
  else
    {
      ...
      for (left_pixels = num_colors / 2;
           (left_pixels < num_pixels - num_colors / 2) && pixels[left_pixels * format + cut_axis] < cut_value;
           left_pixels++)
        ;
      median_cut(pixels, color_table, left_pixels, num_colors / 2, format);
      median_cut(pixels + left_pixels * format, color_table + num_colors / 2 * 3, num_pixels - left_pixels,
                 num_colors / 2, format);
    }
}
...
static int find_symbol_table_index(const LZWEntry *symbol_table, int symbol_index, unsigned char new_byte,
                                   int max_index)
{
  int i;
...
static void write_lzw_symbol(unsigned short symbol, int symbol_length, FILE *fp, long *size_position_p,
                             int *bytes_written_p, unsigned char *buffered_byte_p, int *bit_offset_p)
{
...
static void flush_lzw(FILE *fp, long *size_position_p, int *bytes_written_p, unsigned char *buffered_byte_p,
                      int *bit_offset_p)
{
  unsigned char buffered_byte = *buffered_byte_p;
  int bit_offset = *bit_offset_p;
...
void gif_write(gif_writer *gwp, const unsigned char *rgb_image, unsigned short width, unsigned short height, int format,
               int delay)
{
  ...
  fputc(0x08, gwp->fp); /* 8bit based LZW data */
  {
    ...
    fputc(255, gwp->fp);
    write_lzw_symbol(SYMBOL_CLEAR, max_used_symbol_length, gwp->fp, &size_position, &bytes_written, &buffered_byte,
                     &bit_offset);
        ...
        if (symbol_table_index != SYMBOL_NONE)
        ...
        else
          {
            ...
            if (symbol != SYMBOL_NONE)
              {
                write_lzw_symbol(symbol, max_used_symbol_length, gwp->fp, &size_position, &bytes_written,
                                 &buffered_byte, &bit_offset);
              }
            symbol = new_byte;
          }
      }
    if (symbol != SYMBOL_NONE)
      {
        write_lzw_symbol(symbol, max_used_symbol_length, gwp->fp, &size_position, &bytes_written, &buffered_byte,
                         &bit_offset);
      }
    write_lzw_symbol(SYMBOL_STOP, max_used_symbol_length, gwp->fp, &size_position, &bytes_written, &buffered_byte,
                     &bit_offset);
