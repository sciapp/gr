  ...
  if (wss->s != -1 && wss->wstype == 411)
    {
      ...
      char reply;
      if (!(send_socket(wss->s, &request_type, sizeof(request_type)) == sizeof(request_type) &&
            read_socket(wss->s, &reply, sizeof(reply)) == sizeof(reply) && reply == SOCKET_FUNCTION_IS_ALIVE))
        {
          is_running = 0;
        }
    }
