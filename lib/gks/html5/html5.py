...
fonts = [ "Times New Roman", "Arial", "Courier", "Open Symbol",
...
class Html_output(object):
    ...
    def __init__(self, gks_state_list, data, n):
        ...
        self.p.viewport     = [self.p.width * MWIDTH / WIDTH, 0., self.p.height * MHEIGHT / HEIGHT]
        ...
        self.write('  <body>\n')
        self.write('    <canvas id="html-canvas" width="{0}" height="{1}" tabindex="1" style="outline-style:none;"></canvas>\n'.format(self.p.width, self.p.height))
        self.write('\n')
...
'''
    ...
    def set_clip_rect(self, tnr):
        ...
        if self.gkss.clip == GKS_K_CLIP:
            ...
            self.write('c.save();\n')
            self.write('c.rect({},{},{},{});\n'.format(self.p.clip_rect[0][0], self.p.clip_rect[0][1], self.p.clip_rect[1][0], self.p.clip_rect[1][1]))
            self.write('c.clip();\n')
        else:
            self.p.clip_rect = ()
